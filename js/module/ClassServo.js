...
class ClassServo extends ClassMiddleActuator {
     ...
     * Примечание: метод возвращает данные, зависящие от последней команды, полученной сервоприводом; потому если он не был надлдежащим образом откалиброван, либо его положение было изменено в обход методов, прдставленных в ModuleServo, полученные значения вероятно не будут соответствовать действительности
     * @returns 
     */
    GetInfo() {
        return ({
